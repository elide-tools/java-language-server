...
public class Source {
     ...
     * (even if a path is specified). A sourceReference is only valid for a session, so it must not
     * be used to persist a source.
     */
    public Integer sourceReference;

    /**
     * An optional hint for how to present the source in the UI. A value of 'deemphasize' can be
     * used to indicate that the source is not available or that it is skipped on stepping. 'normal'
